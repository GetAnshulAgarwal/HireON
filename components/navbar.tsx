...
export default function Navbar({ userType = "candidate" }: NavbarProps) {
  ...
  const handleImageError = () => {
    if (!imageError) {
      setProfilePic("/images/default-profile.png");
      setImageError(true);
    }
  };
  ...
  const savePreferences = async () => {
    ...
    try {
      const res = await fetch(
        "https://hiring-platform-beta.onrender.com/api/auth/preferences",
        {
          method: "PUT",
          headers: {
      ...
      const message = err instanceof Error ? err.message : "Unknown error";
      toast.error(`Error saving preferences: ${message}`);
      if (message.includes("401")) {
        localStorage.removeItem("token");
        router.push("/");
      }
  ...
  const profileSrc = profilePic.startsWith("http")
    ? profilePic
    : `https://hiring-platform-beta.onrender.com${profilePic}`;
        ...
        <div className="flex items-center">
          ...
          >
            <span className="text-xl font-bold text-primary">
              Hire<span className="text-white">ON</span>
            </span>
          </Link>
        </div>
