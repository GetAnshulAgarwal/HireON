...
export default function RecruiterDashboard() {
  ...
  const fetchJobs = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        "https://hiring-platform-beta.onrender.com/api/jobs/recruiter",
        {
      ...
      const newApplicants = data.reduce(
        (sum, job) => sum + (job.newApplicantsCount || 0),
        0
      );
      setStats({
  ...
  const handleClose = async (jobId: string) => {
    ...
    try {
      const res = await fetch(
        `https://hiring-platform-beta.onrender.com/api/jobs/${jobId}/close`,
        {
          method: "PUT",
          headers: {
  ...
  const handleDelete = async (jobId: string) => {
    ...
    try {
      const res = await fetch(
        `https://hiring-platform-beta.onrender.com/api/jobs/${jobId}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        }
        ...
        <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 animate-fade-in">
          ...
          <div className="card p-6 flex items-center">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-foreground">{stats.newApplicants}</p>
            </div>
          </div>
        </section>
