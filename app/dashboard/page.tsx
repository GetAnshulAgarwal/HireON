...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (!jobsRes.ok)
          throw new Error(`Jobs fetch failed with status: ${jobsRes.status}`);
        if (!appsRes.ok)
          throw new Error(`Applications fetch failed with status: ${appsRes.status}`);
  ...
  const handleApply = async (jobId: string): Promise<void> => {
    ...
    try {
      const extractRes = await fetch(
        "https://hiring-platform-beta.onrender.com/api/resume/extract",
        {
          method: "POST",
          headers: { Authorization: `Bearer ${token}` },
          body: formData,
      ...
      if (!extractRes.ok)
        throw new Error(`Resume extraction failed with status: ${extractRes.status}`);
      const { resumeText } = await extractRes.json();

      const applyRes = await fetch(
        "https://hiring-platform-beta.onrender.com/api/applications/apply",
        {
          method: "POST",
          headers: {
      ...
      if (!applyRes.ok)
        throw new Error(`Application submission failed with status: ${applyRes.status}`);
      const newApplication: Application = await applyRes.json();
  ...
  const handleChat = async (application: Application): Promise<void> => {
    ...
    try {
      const res = await fetch(
        `https://hiring-platform-beta.onrender.com/api/chat/${application._id}`,
        {
  ...
  const sendMessage = async (): Promise<void> => {
    ...
    try {
      const res = await fetch(
        `https://hiring-platform-beta.onrender.com/api/chat/${showChatModal.application}`,
        {
          method: "POST",
          headers: { Authorization: `Bearer ${token}` },
          body: formData,
  ...
  const getStatusIcon = (status: Application["status"]): React.ReactElement => {
    switch (status) {
      case "Applied":
      ...
      <main className="flex-1 p-6 max-w-7xl mx-auto w-full">
        <div className="bg-accent p-8 rounded-lg mb-8 shadow-md relative overflow-hidden">
          ...
          <p className="text-gray-300 max-w-2xl relative z-10">
            Your smart hiring platform. Find the perfect job match with AI-powered
            ...
            {jobs.slice(0, 6).map((job, index) => (
              ...
              >
                ...
                <div className="mt-auto pt-3 border-t border-border flex justify-end">
                  <button
                    onClick={() =>
                      appliedJobs.includes(job._id) ? null : setShowApplyModal(job)
                    }
        ...
        <section className="mb-12 animate-fade-in" style={{ animationDelay: "0.3s" }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            ...
            <div className="card hover:border-primary group cursor-pointer">
              <div className="p-3 rounded-full bg-primary bg-opacity-20 inline-block mb-4 group-hover:bg-primary group-hover:bg-opacity-30 transition-all">
            ...
            <div className="bg-accent p-6 rounded-lg shadow-lg w-full max-w-lg modal-content">
              <h2 className="text-xl font-bold text-primary mb-4 flex items-center gap-2">
                ...
                <span>
                  Chat for{" "}
                  {
                    applications.find((app) => app._id === showChatModal.application)
                      ?.job.title
                  }
                </span>
              </h2>
